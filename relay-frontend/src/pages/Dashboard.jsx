...
import NetworkFlowPanel from '../components/NetworkFlowPanel';
import IdentityPanel from '../components/IdentityPanel';
import NodePanel from '../components/NodePanel';
import RelayCreation from '../components/RelayCreation';
...
export default function Dashboard() {
  ...
  const [appNodes, setAppNodes] = useState([]); // Multiple Nodes

  // Fetch initial relays and nodes
  ...
  const fetchNodes = useCallback(async () => {
    try {
      ...
      // OPTIONAL AUTO SETUP from phase 4
      if (nodeData.length < 2) {
        console.log("Auto-creating nodes to reach minimum 2 nodes for sending...");
        await API.registerNode();
        if (nodeData.length === 0) {
          await API.registerNode();
        }
        // Fetch again after auto creation
        const newRes = await API.getAllNodes();
        nodeData = newRes.data?.data || [];
      }

      setAppNodes(Array.isArray(nodeData) ? nodeData : []);
      
      // Auto-select source and destination if empty and nodes exist
      if (nodeData.length >= 2) {
        if(!source) setSource(nodeData[0].nodeId);
        if(!destination) setDestination(nodeData[1].nodeId);
      } else if (nodeData.length === 1 && !source) {
        setSource(nodeData[0].nodeId);
      }
    } catch (e) { 
        console.error("Failed to fetch app nodes", e);
    }
  }, [source, destination]);
  ...
  useEffect(() => { 
    ...
    fetchNodes();
    fetchMessages();
  }, [fetchRelays, fetchNodes, fetchMessages]);

  // WebSocket Integration (Real-time System)
  useEffect(() => {
    const socket = new SockJS('http://localhost:8080/ws-relay');
    const stompClient = Stomp.over(socket);
    stompClient.debug = () => {}; // disable debug logs

    stompClient.connect({}, () => {
      // Listen for metric updates (Leaderboard, Identity, Status)
      ...
      stompClient.subscribe('/topic/messages', (message) => {
        const cur = JSON.parse(message.body);
        setMessages(prev => [cur, ...prev].slice(0, 50));
      });
    });
        ...
        <header className="border-b border-white/5 px-6 py-4">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={handleLogout} className="px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg text-xs font-semibold transition-colors">
                Logout
              </button>
            </div>
          </div>
        ...
        <main className="max-w-7xl mx-auto px-6 py-6 space-y-6">
          ...
          {activeTab === 'dashboard' && (
            <>
              ...
              {/* Module 1: Node Identity Generation Panel */}
              <div className="mt-6">
                <IdentityPanel relays={relays} />
              </div>
            </>
          )}
          ...
          {activeTab === 'nodes' && (
            <div className="space-y-6">
              <NodePanel />
              <RelayCreation onRelayCreated={fetchRelays} />
            </div>
          )}
          ...
          {activeTab === 'send' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="glass rounded-2xl p-6 space-y-4">
                ...
                <div className="space-y-3">
                  {appNodes.length < 2 ? (
                    <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg text-amber-400 text-sm">
                      ⚠️ Create at least 2 nodes to send messages. (Auto-creating...)
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="z-10 relative">
                        ...
                        <CustomDropdown 
                           ...
                           options={appNodes.filter(n => n.nodeId !== source)} 
                ...
                <button onClick={sendMessage} disabled={loading || appNodes.length < 2 || !source || !destination}
              ...
              <div className="space-y-4">
                ...
                {/* Module 4: Network Flow Topology Visualization */}
                <div className="pt-4 z-0 relative">
                  <NetworkFlowPanel 
                    activePath={result?.success ? (result.data?.hopPath && result.data.hopPath.length > 0 ? result.data.hopPath : [result.data?.relayNode]) : []} 
                    relays={relays} 
                    messages={messages} 
                    sourceNode={source}
